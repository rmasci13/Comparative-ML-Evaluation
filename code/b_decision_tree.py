...
def tuning(X_train, y_train, cv):
    ...
    plt.ylabel('F1 Score')
    plt.title('Decision Tree Max Depth Tuning Curve - Baseball')
    plt.legend()
    plt.savefig('../plots/b_dt_max_depth_tuning.png')
    plt.close()
...
def dt_run(X_train, y_train, cv):
    ...
    print("Start DT Baseball")
    best_model, train_f1_score, val_f1_score = tuning(X_train, y_train, cv)
    create_learning_curve(best_model, X_train, y_train, cv, 'DT Optimal Model Learning Curve - Baseball', '../plots/b_dt_learning_curve_optimal.png')
    print(f"Done DT Baseball - Training F1: {train_f1_score}, Validation F1: {val_f1_score}")
